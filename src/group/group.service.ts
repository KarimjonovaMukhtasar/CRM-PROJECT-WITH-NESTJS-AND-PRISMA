  ...
  constructor(private readonly prisma: PrismaService) {}
      async create(createGroupDto: CreateGroupDto) {
        try {
          ...
          if (!checkCourseId) throw new NotFoundException(`NOT FOUND SUCH A COURSE ID!`);
      ...
      async remove(id: string, force: boolean) {
        try {
           const checkId = await this.prisma.group.findUnique({where: {id}})
                 ...
                 const checkCourses = await this.prisma.course.findMany({where: {id: checkId.courseId}})
                 if(checkCourses.length === 0) {
                   ...
                   if(force){
                     await this.prisma.course.deleteMany({where: {id: checkId.courseId}})
                     return {
                       success: true,
                   ...
                   throw new BadRequestException(`THIS GROUP HAVE COURSES, ARE YOU SURE TO DELETE ALL OF THEM ALONG WITH THE GROUP?`)
